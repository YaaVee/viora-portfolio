...
echo "========================================="
echo "🚑 EMERGENCY FIX FOR RAW FUNCTION DISPLAY"
echo "========================================="
...
echo "✅ Backup saved as index.html.broken"

# Extract everything before the broken function
sed -n '1,/function getHomePage()/p' index.html | head -n -1 > index.html.fixed

# Add the properly formatted getHomePage function
cat >> index.html.fixed << 'FUNC'
...
# Add the rest of the file after the function
AFTER_FUNC=$(grep -n "function getHomePage" index.html | head -1 | cut -d: -f1)
tail -n +$((AFTER_FUNC + 20)) index.html | sed -n '/^        function/,$p' >> index.html.fixed

# Replace original with fixed
mv index.html.fixed index.html

echo "✅ Emergency fix applied!"
echo "🌐 Open index.html in browser to test"
